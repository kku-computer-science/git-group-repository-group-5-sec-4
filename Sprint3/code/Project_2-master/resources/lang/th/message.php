<?php
return [
'Home' => 'หน้าแรก',
'Researchers' => 'ผู้วิจัย',
'ResearchProj' => 'โครงการวิจัย',
'ResearchGroup' => 'กลุ่มวิจัย',
'Report' => 'รายงาน',
'details' =>'รายละเอียดเพิ่มเติม',
'expertise' =>'ความเชี่ยวชาญ',
'publications' => 'ผลงานตีพิมพ์ (5 ปี ย้อนหลัง)',
'education'=>'การศึกษา',
'publications2' => 'ผลงานตีพิมพ์',
'project_service' => 'โครงการบริการวิชาการ/ โครงการวิจัย',
'no' => 'ลำดับ',
'year' => 'ปี',
'project_name' => 'ชื่อโครงการ',
'details' => 'รายละเอียด',
'project_responsible' => 'ผู้รับผิดชอบโครงการ',
'status' => 'สถานะ',
'project_duration' => 'ระยะเวลาโครงการ',
'to' => 'ถึง',
'research_fund_type' => 'ประเภททุนวิจัย',
'funding_agency' => 'หน่วยงานที่สนับสนุนทุน',
'responsible_department' => 'หน่วยงานที่รับผิดชอบ',
'budget_allocated' => 'งบประมาณที่ได้รับจัดสรร',
'baht' => 'บาท',
'submitted' => 'ยื่นขอ',
'in_progress' => 'ดำเนินการ',
'closed' => 'ปิดโครงการ',
'article_stats' => 'สถิติจำนวนบทความทั้งหมด 5 ปี',
'citation_stats' => 'สถิติจำนวนบทความที่ได้รับการอ้างอิง',
'Login' => 'เข้าสู่ระบบ',
'total_articles_report' => 'รายงานจำนวนบทความทั้งหมด (5 ปี: สะสม)',
'number_chart' => 'จำนวนงานวิจัย',
'search' => 'ค้นหา',
'show_entries' => 'แสดง :entries รายการ',
'showing_entries' => 'แสดง :start ถึง :end จากทั้งหมด :total รายการ',
'first' => 'หน้าแรก',
'last' => 'หน้าสุดท้าย',
'next' => 'ถัดไป',
'previous' => 'ก่อนหน้า',
'research_interests' => 'หัวข้อที่สนใจในการวิจัย',
'before' => 'ก่อน',
'reference' => 'อ้างอิง',
'summary' => 'สรุป',
    'project_service' => 'โครงการบริการวิชาการ/ โครงการวิจัย',
    'no' => 'ลำดับ',
    'year' => 'ปี',
    'project_name' => 'ชื่อโครงการ',
    'details' => 'รายละเอียด',
    'project_responsible' => 'ผู้รับผิดชอบโครงการ',
    'status' => 'สถานะ',
    'project_duration' => 'ระยะเวลาโครงการ',
    'to' => 'ถึง',
    'research_fund_type' => 'ประเภททุนวิจัย',
    'funding_agency' => 'หน่วยงานที่สนับสนุนทุน',
    'responsible_department' => 'หน่วยงานที่รับผิดชอบ',
    'budget_allocated' => 'งบประมาณที่ได้รับจัดสรร',
    'baht' => 'บาท',
    'submitted' => 'ยื่นขอ',
    'in_progress' => 'ดำเนินการ',
    'closed' => 'ปิดโครงการ',
    'Summary'=> 'งานวิจัยทั้งหมด',
    'ExportToExcel' => 'ส่งออกไฟล์ Excel',
    'No' => 'ลำดับ',
    'Year' => 'ปี',
    'PaperName' => 'ชื่อบทความ',
    'Author' => 'ผู้แต่ง',
    'DocumentType' => 'ประเภทเอกสาร',
    'Page' => 'หน้าที่',
    'Journals/Transactions' => 'วารสารที่ตีพิมพ์',
    'Ciations' => 'การอ้างอิง',
    'Doi' => 'รหัสที่ใช้ระบุเอกสารดิจิตอล',
    'Source' => 'แหล่งที่มา',
    'Number' => 'ลำดับ',
    'Name' => 'ชื่อหนังสือ',
    'Placeofpublication' => 'สถานที่พิมพ์',
    'Type' => 'ประเภท',
    'Registrationnumber' => 'หมายเลขทะเบียน',
    'Dateofregistration' => 'วันที่จดทะเบียน',
    'Search' => 'ค้นหา',
    'Book' => 'หนังสือ',
    'Otheracademicworks' => 'ผลงานวิชาการด้านอื่นๆ',
    'laboratory_supervisor' => 'หัวหน้าห้องปฏิบัติการ',
   'student' => 'นักศึกษา',
   'reference_apa' => 'เอกสารอ้างอิง (APA)',
    'close' => 'ปิด',
    'modal_body_text' => 'ข้อความในโมดอลแสดงที่นี่',
    // ส่วนของ Login
'Account_Login' => 'เข้าสู่ระบบ',
'Username' => 'ชื่อผู้ใช้',
'Password' => 'รหัสผ่าน',
'Remember_me' => 'จดจำบัญชีผู้ใช้',
'Login_button' => 'เข้าสู่ระบบ',
'Forgot_password' => '*** หากลืมรหัสผ่าน ให้ติดต่อผู้ดูแลระบบ',
'Username_kkumail' => 'สำหรับชื่อผู้ใช้ ใช้ KKU-Mail ในการเข้าสู่ระบบ',
'Student_first_time_login' => 'สำหรับนักศึกษาที่เข้าระบบเป็นครั้งแรกให้เข้าสู่ระบบด้วยรหัสนักศึกษา',
'Login_failed' => ' เข้าสู่ระบบล้มเหลว: ชื่อผู้ใช้หรือรหัสผ่านของคุณไม่ถูกต้อง',
'required' => [
        'username' => 'ต้องระบุชื่อผู้ใช้',
        'password' => 'ต้องระบุรหัสผ่าน'
],
// ส่วนของ Admin (ผู้วิจัย)

// ส่วนของ Pagination
'Previous' => 'ก่อนหน้า',
'Next' => 'ถัดไป',


// ========================================== ส่วนของ Admin (ผู้วิจัย) ==========================================

// ส่วน Navbar
'Project_title' => 'ระบบจัดการข้อมูลวิจัยของสาขาวิชาวิทยาการคอมพิวเตอร์',
'Search_here' => 'ค้นหาที่นี่',
'Logout' => 'ออกจากระบบ',

// ส่วนของหน้า Dashboard
'Welcome' => 'ยินดีต้อนรับเข้าสู่ระบบจัดการข้อมูลวิจัยของสาขาวิชาวิทยาการคอมพิวเตอร์',
'Welcome_hello' => 'สวัสดี',
'Dashboard_navbar_title' => 'แดชบอร์ด',

// ส่วนของ Profile
'Profile_navbar_title' => 'โปรไฟล์',
'position' => 'ตำแหน่ง',
// ส่วนของ Account
'Profile_Account_ProfileSetting' => 'การตั้งค่าโปรไฟล์',
'Profile_Account_pronouns' => 'คำนำหน้า',
'Profile_Account_firstnameEng' => 'ชื่อ (ภาษาอังกฤษ)',
'Profile_Account_lastnameEng' => 'นามสกุล (ภาษาอังกฤษ)',
'Profile_Account_firstnameTH' => 'ชื่อ (ภาษาไทย)',
'Profile_Account_lastnameTH' => 'นามสกุล (ภาษาไทย)',
'Profile_Account_firstnameCN' => 'ชื่อ (ภาษาจีน)',
'Profile_Account_lastnameCN' => 'นามสกุล (ภาษาจีน)',
'Profile_Account_email' => 'อีเมล์',
'Profile_Account_AcademicRank' => 'ยศศักดิ์ทางวิชาการ',
'Profile_Account_AcademicPosition' => 'ตำแหน่งทางวิชาการ',
'Profile_Account_TogglePHD' => 'สำหรับอ.ผู้ที่ไม่มีคุณวุฒิปริญญาเอก โปรดระบุ',
'Profile_Account_Professor' => 'ศาสตราจารย์',
'Profile_Account_AssociateProfessor' => 'รองศาสตราจารย์',
'Profile_Account_AssistantProfessor' => 'ผู้ช่วยศาสตราจารย์',
'Profile_Account_Lecturer' => 'อาจารย์',

'Profile_All_Update' => 'อัพเดท',

'Organization_support_en' => 'องค์กรสนับสนุน/โครงการวิจัย (ภาษาอังกฤษ)',
'Organization_support_cn' => 'องค์กรสนับสนุน/โครงการวิจัย (ภาษาจีน)',

// ส่วนของ User Profile
'User_Profile_navbar_title' => 'โปรไฟล์ของผู้ใช้',
// ส่วนของ User Profile
'User_Profile_navbar_title' => 'โปรไฟล์ของผู้ใช้',
'Change_picture' => 'เปลี่ยนรูปภาพ',
'User_profile_picture' => 'รูปโปรไฟล์ของผู้ใช้',
'Account_tab' => 'บัญชี',
'Password_tab' => 'รหัสผ่าน',
'Expertise_tab' => 'ความเชี่ยวชาญ',
'Education_tab' => 'การศึกษา',
'Profile_settings' => 'การตั้งค่าโปรไฟล์',
'Not_hold_doctoral_degree' => 'สำหรับอ.ผู้ที่ไม่มีคุณวุฒิปริญญาเอก โปรดระบุ',
'Update_button' => 'อัพเดท',
'Password_settings' => 'การตั้งค่ารหัสผ่าน',
'Old_password' => 'รหัสผ่านเดิม',
'New_password' => 'รหัสผ่านใหม่',
'Confirm_new_password' => 'ยืนยันรหัสผ่านใหม่',
'Expertise_title' => 'ความเชี่ยวชาญ',
'Add_expertise_button' => 'เพิ่มความเชี่ยวชาญ',
'Expertise_title_form' => 'ความเชี่ยวชาญ',
'Expertise_placeholder_form' => 'กรอกความเชี่ยวชาญ',
'Submit_button' => 'ยืนยัน',
'Cancle_button' => 'ยกเลิก',
'Education_title' => 'การศึกษา',
'Bachelor_degree_title' => 'ปริญญาตรี',
'Master_degree_title' => 'ปริญญาโท',
'Doctoral_degree_title' => 'ปริญญาเอก',
'University_name' => 'ชื่อมหาวิทยาลัย',
'Degree_title' => 'ชื่อวุฒิปริญญา',
'Year_graduation' => 'ปี พ.ศ. ที่จบ',

// ส่วนของ Option
'Option_navbar_title' => 'ตัวเลือก',

// ส่วนของ Manage Fund
'Manage_Fund_navbar_title' => 'จัดการแหล่งทุนวิจัย',

// ส่วนของ Research Project
'Research_Project_navbar_title' => 'โครงการวิจัย',
// ส่วน Manage Fund (index.blade.php)
'Research_grant' => 'ทุนวิจัย',
'Add_research_grant' => 'เพิ่ม',
'Fund_no' => 'ลำดับที่',
'Fund_name' => 'ชื่อทุน',
'Fund_type' => 'ประเภททุน',
'Fund_level' => 'ระดับทุน',
'Fund_action' => 'ดำเนินการ',
'No_data_avalible' => 'ไม่มีข้อมูลในตาราง',
'info' => 'แสดง _START_ ถึง _END_ จาก _TOTAL_ รายการ',
'infoEmpty' => 'แสดง 0 ถึง 0 จาก 0 รายการ',
'infoFiltered' => '(กรองจาก _MAX_ รายการทั้งหมด)',
'lengthMenu' => 'แสดง _MENU_ รายการ',
'search' => 'ค้นหา:',
'internal_fund' => 'ทุนภายใน',
'external_fund' => 'ทุนภายนอก',

// ส่วน Manage Fund (show.blade.php)
'Fund_detail' => 'รายละเอียดทุน',
'Fund_detail_description' => 'รายละเอียดของทุน',
'Fund_year' => 'ปีที่ได้รับทุน',
'Fund_organization' => 'องค์กรที่ให้ทุน',
'Add_detail_fund_by' => 'เพิ่มรายละเอียดทุนโดย',
'Back_button' => 'กลับ',

// ส่วน Manage Fund (create.blade.php)
'error_input' => [
    'Whoops' => 'อุปส์!',
    'Error_problem' => 'มีปัญหาเกิดขึ้นเกี่ยวกับข้อมูลที่คุณป้อน',
],
'Add_research_fund' => 'เพิ่มทุนวิจัย',
'Input_research_fund_detail' => 'กรอกรายละเอียดทุนวิจัย',
'Please_choose_fund_type' => 'กรุณาเลือกประเภททุน',
'Please_choose_fund_level' => 'กรุณาเลือกระดับทุน',
'Organization_support' => 'องค์กรที่สนับสนุน / โครงการวิจัย (ภาษาไทย)',
'Fund_level_not_define' => 'ไม่ระบุ',
'Fund_level_low' => 'ล่าง',
'Fund_level_medium' => 'กลาง',
'Fund_level_high' => 'สูง',


// ส่วน Manage Fund (edit.blade.php)
'Edit_fund' => 'แก้ไขทุน',
'Edit_research_fund_detail' => 'แก้ไขรายละเอียดทุน',

// ส่วน Manage Fund (การ Delete)
'Fund_warning_delete' => [
    'warning_title' => 'คุณแน่ใจหรือไม่?',
    'warning_text' => 'คุณจะไม่สามารถกู้คืนข้อมูลที่ถูกลบไป!',
],


// ส่วนของ Research Project
'Research_Project_navbar_title' => 'โครงการวิจัย',
// ส่วนของ Research Project (index.blade.php)
'Add_research_project' => 'เพิ่ม',
'Research_project_no' => 'ลำดับที่',
'Research_project_year' => 'ปี',
'Research_project_name' => 'ชื่อโครงการ',
'Research_project_head' => 'หัวหน้าโครงการ',
'Research_project_member' => 'สมาชิก',
'Research_project_action' => 'ดำเนินการ',

// ส่วนของ Research Projects (show.blade.php)
'Research_project_detail' => 'รายละเอียดโครงการ',
'Research_project_description' => 'รายละเอียดของโครงการ',
'Research_project_start' => 'วันที่เริ่ม',
'Research_project_end' => 'วันที่สิ้นสุด',
'Research_project_budget' => 'งบประมาณ',
'Research_project_note' => 'คำอธิบาย',
'Research_project_status' => [
    'Title' => 'สถานะ',
    'Request' => 'ยื่นขอ',
    'Progress' => 'กำลังดำเนินการ',
    'Close' => 'ปิดโครงการ',
],
'Research_project_responsible' => 'ผู้รับผิดชอบโครงการ',

// ส่วนของ Research Projects (create.blade.php)
'Input_research_project_detail' => 'กรอกรายละเอียดโครงการวิจัย',
'Please_choose_fund' => 'เลือกทุน',
'Year_submission' => 'ปีที่ยื่น (ค.ศ.)',
'Budget_baht_placeholder' => 'งบประมาณ (บาท)',
'Responsible_department' => 'หน่วยงานที่รับผิดชอบ',
'Select_department_option' => 'เลือกสาขาวิชา',
'Please_choose_status' => 'เลือกสถานะ',
'Select_user_option' => 'เลือกผู้ใช้',
'Internal_co-project_manager' => 'ผู้รับผิดชอบโครงการ (ร่วม) ภายใน',
'External_co-project_manager' => 'ผู้รับผิดชอบโครงการ (ร่วม) ภายนอก',
'Postion_or_prefix' => 'ตำแหน่งหรือคำนำหน้า',
'First_name' => 'ชื่อ',
'Last_name' => 'นามสกุล',
'Add_more_person' => 'เพิ่มผู้รับผิดชอบเพิ่มเติม',
'Record_inserted_successfully' => 'บันทึกข้อมูลเรียบร้อยแล้ว',
'Cant_remove_first_row' => 'ขออภัย!! ไม่สามารถลบแถวแรกได้!',

// ส่วนของ Research Projects (edit.blade.php)
'Edit_research_project' => 'แก้ไขโครงการวิจัย',
'Edit_research_project_detail' => 'แก้ไขรายละเอียดโครงการ',

// ส่วนของ Research Group
'Research_Group_navbar_title' => 'กลุ่มวิจัย',
'Research_group_name_cn' => 'ชื่อกลุ่มวิจัย (ภาษาจีน)',
'Research_group_description_cn' => 'คำอธิบายกลุ่มวิจัย (ภาษาจีน)',
'Research_group_detail_cn' => 'รายละเอียดกลุ่มวิจัย (ภาษาจีน)',

// ส่วนของ Research Group (index.blade.php)
'Add_research_group' => 'เพิ่ม',
'Research_group_no' => 'ลำดับที่',
'Research_group_name' => 'ชื่อกลุ่มวิจัย',
'Research_group_head' => 'หัวหน้ากลุ่มวิจัย',
'Research_group_member' => 'สมาชิก',
'Research_group_action' => 'ดำเนินการ',

// ส่วนของ Research Group (show.blade.php)
'Research_group_detail' => 'รายละเอียดกลุ่มวิจัย',
'Research_group_description' => 'รายละเอียดของกลุ่มวิจัย',
'Research_group_name_th' => 'ชื่อกลุ่มวิจัย (ไทย)',
'Research_group_name_en' => 'ชื่อกลุ่มวิจัย (อังกฤษ)',
'Research_group_description_th' => 'คำอธิบายกลุ่มวิจัย (ไทย)',
'Research_group_description_en' => 'คำอธิบายกลุ่มวิจัย (อังกฤษ)',
'Research_group_detail_th' => 'รายละเอียดกลุ่มวิจัย (ไทย)',
'Research_group_detail_en' => 'รายละเอียดกลุ่มวิจัย (อังกฤษ)',
'Research_group_head' => 'หัวหน้ากลุ่มวิจัย',
'Research_group_member' => 'สมาชิกกลุ่มวิจัย',

// ส่วนของ Research Group (create.blade.php)
'Create_research_group' => 'สร้างกลุ่มวิจัย',
'Input_research_group_detail' => 'กรอกรายละเอียดกลุ่มวิจัย',
'Research_group_image' => 'รูปภาพกลุ่มวิจัย',
'Department_name' => 'สาขาวิชาวิทยาการคอมพิวเตอร์',

// ส่วนของ Research Group (edit.blade.php)
'Edit_research_group' => 'แก้ไขกลุ่มวิจัย',
'Edit_research_group_detail' => 'แก้ไขรายละเอียดกลุ่มวิจัย',

// ส่วนของ Manage Publication
'Manage_Publication_navbar_title' => 'จัดการผลงานตีพิมพ์',

// ส่วนของ Published research
'Published_research_navbar_title' => 'ผลงานวิจัยที่ตีพิมพ์',

// ส่วนของ Book
'Book_navbar_title' => 'หนังสือ',

// ส่วนของ ผลงานวิชาการอื่นๆ
'Other_academic_works_navbar_title' => 'ผลงานวิชาการอื่นๆ',

'No_data_avalible' => 'ไม่มีข้อมูลในตารางนี้',
'info' => '',
'infoEmpty' => 'กำลังโชว์ข้อมูลว่างเปล่า',
'infoFiltered' => '(กรองจากจำนวนทั้งหมด)',
'lengthMenu' => '',
'search' => 'ค้นหา:',
'Previous' => 'ย้อนหลัง',
'Next' => 'ถัดไป',

// ส่วนของ Published research (index.blade.php)
'Add_published_research' => 'เพิ่ม',
'Call_paper_allAPI' => 'Call Paper (ทุก API)',
'Call_paper_scopus' => 'Call Paper (Scopus)',
'Call_paper_googlescholar' => 'Call Paper (Google Scholar)',
'Call_paper_wos' => 'Call Paper (Web of Science)',
'Published_research_no' => 'ลำดับที่',
'Published_research_title' => 'ชื่อผลงาน',
'Published_research_type' => 'ประเภท',
'Published_research_year' => 'ปีที่ตีพิมพ์',
'Published_research_API_fetch_date' => 'วันที่ดึงข้อมูลจาก API',
'Published_research_action' => 'ดำเนินการ',
'Published_research_journal' => 'วารสาร',
'Published_research_conference' => 'การประชุมวิชาการ',
'Published_research_book_series' => 'ชุดหนังสือ',
'Published_research_book' => 'หนังสือ',

// ส่วนของ Published research (show.blade.php)
'Published_research_detail' => 'รายละเอียดผลงานวิจัยที่ตีพิมพ์',
'Published_research_description' => 'รายละเอียดของผลงาน',
'Published_research_abstract' => 'บทคัดย่อ',
'Published_research_keyword' => 'คำสำคัญ',
'Published_research_journalType' => 'ประเภทวารสาร',
'Published_research_documentType' => 'ประเภทเอกสาร',
'Published_research_publication' => 'การตีพิมพ์',
'Published_research_author' => 'ผู้แต่ง',
'Published_research_first_author' => 'ผู้แต่งคนแรก',
'Published_research_co_author' => 'ผู้แต่งร่วม',
'Published_research_corresponding_author' => 'ผู้แต่งที่สามารถติดต่อได้',
'Published_research_journalName' => 'ชื่อวารสาร',
'Published_research_volume' => 'เล่ม',
'Published_research_issue' => 'ฉบับ',
'Published_research_page' => 'หน้า',
'Published_research_doi' => 'DOI',
'Published_research_url' => 'URL',

// ส่วนของ Published research (create.blade.php)
'Create_published_research' => 'สร้างผลงานวิจัยที่ตีพิมพ์',
'Input_published_research_detail' => 'กรอกรายละเอียดผลงานวิจัยที่ตีพิมพ์',
'Published_research_source' => 'แหล่งเผยแพร่งานวิจัย',
'Published_research_keyword_suggested' => 'แต่ละคำสำคัญต้องถูกคั่นด้วยเครื่องหมายจุลภาค (,)',
'Choose_paper_type' => 'เลือกประเภทของวารสาร',
'Choose_paper_subtype' => 'เลือกประเภทของเอกสาร',
'Choose_publication' => 'เลือกการตีพิมพ์',
'Published_research_citation_count' => 'จำนวนการอ้างอิง',
'Published_research_funder' => 'ผู้สนับสนุนทุน',
'Published_research_internal_author' => 'ชื่อผู้แต่ง (ในสาขา)',
'Published_research_external_author' => 'ชื่อผู้แต่ง (นอกสาขา)',
'Enter_your_name' => 'กรอกชื่อของคุณ',

'Published_research_document_type_article' => 'บทความ',
'Published_research_document_type_conference' => 'การประชุมวิชาการ',
'Published_research_document_type_editorial' => 'บทบรรณาธิการ',
'Published_research_document_type_bookchapter' => 'บทความในหนังสือ',
'Published_research_document_type_erratum' => 'ข้อบกพร่อง',
'Published_research_document_type_review' => 'บทวิจารณ์',

'Published_research_publication_international_journal' => 'วารสารนานาชาติ',
'Published_research_publication_international_book' => 'หนังสือนานาชาติ',
'Published_research_publication_international_conference' => 'การประชุมวิชาการนานาชาติ',
'Published_research_publication_national_conference' => 'การประชุมวิชาการในประเทศ',
'Published_research_publication_national_journal' => 'วารสารในประเทศ',
'Published_research_publication_national_book' => 'หนังสือในประเทศ',
'Published_research_publication_national_magazine' => 'นิตยสารในประเทศ',
'Published_research_publication_book_chapter' => 'บทความในหนังสือ',

// ส่วนของ Published research (edit.blade.php)
'Edit_published_research' => 'แก้ไขผลงานวิจัยที่ตีพิมพ์',
'Edit_published_research_detail' => 'แก้ไขรายละเอียดผลงานวิจัยที่ตีพิมพ์',

// ส่วนของ Book
'Book_navbar_title' => 'หนังสือ',

// ส่วนของ Book (index.blade.php)
'Add_book' => 'เพิ่ม',
'Book_no' => 'ลำดับที่',
'Book_title' => 'ชื่อหนังสือ',
'Book_year' => 'ปีที่ตีพิมพ์ (พ.ศ.)',
'Book_source' => 'แหล่งเผยแพร่หนังสือ(ภาษาไทย)',
'Book_source_en' => 'แหล่งเผยแพร่หนังสือ(อังกฤษ)',
'Book_source_cn' => 'แหล่งเผยแพร่หนังสือ(จีน)',
'Book_page' => 'หน้า',
'Book_action' => 'ดำเนินการ',

// ส่วนของ Book (show.blade.php)
'Book_detail' => 'รายละเอียดหนังสือ',
'Book_description' => 'รายละเอียดของหนังสือ',

// ส่วนของ Book (create.blade.php)
'Create_book' => 'สร้างหนังสือ',
'Input_book_detail' => 'กรอกรายละเอียดหนังสือ',

// ส่วนของ Book (edit.blade.php)
'Edit_book' => 'แก้ไขหนังสือ',
'Edit_book_detail' => 'แก้ไขรายละเอียดหนังสือ',

// ส่วนของ ผลงานวิชาการอื่นๆ
'Other_academic_works_navbar_title' => 'ผลงานวิชาการอื่นๆ',

// ส่วนของ ผลงานวิชาการอื่นๆ (index.blade.php)
'Other_academic_works_header' => 'ผลงานวิชาการอื่นๆ (สิทธิบัตร, อนุสิทธิบัตร,ลิขสิทธิ์)',
'Add_other_academic_works' => 'เพิ่ม',
'Other_academic_works_no' => 'ลำดับที่',
'Other_academic_works_title' => 'ชื่อผลงาน',
'Other_academic_works_type' => 'ประเภท',
'Other_academic_registration_date' => 'วันที่จดทะเบียน',
'Other_academic_registration_no' => 'หมายเลขจดทะเบียน',
'Other_academic_works_author' => 'จัดทำโดย',
'Other_academic_works_action' => 'ดำเนินการ',
'Delete_successfully' => 'ลบข้อมูลสำเร็จแล้ว',

// ส่วนของ ผลงานวิชาการอื่นๆ (show.blade.php)
'Other_academic_works_detail' => 'รายละเอียดผลงานวิชาการอื่นๆ (สิทธิบัตร, อนุสิทธิบัตร,ลิขสิทธิ์)',
'Other_academic_works_description' => 'รายละเอียดของผลงาน (สิทธิบัตร, อนุสิทธิบัตร,ลิขสิทธิ์)',
'Other_academic_works_co-author' => 'ผู้จัดทำร่วม',
'Other_academic_registration_number' => 'หมายเลข',

// ส่วนของ ผลงานวิชาการอื่นๆ (create.blade.php)
'Create_other_academic_works' => 'สร้างผลงานวิชาการอื่นๆ',
'Input_other_academic_works_detail' => 'กรอกรายละเอียดผลงานวิชาการอื่นๆ (สิทธิบัตร, อนุสิทธิบัตร,ลิขสิทธิ์)',
'Other_academic_works_create_title' => 'ชื่อผลงาน (สิทธิบัตร, อนุสิทธิบัตร,ลิขสิทธิ์)',
'Other_academic_works_choose_type' => 'เลือกประเภท',
'Other_academic_works_date_copyright' => 'วันที่จดทะเบียนลิขสิทธิ์',
'Other_academic_works_teacher_in_field' => 'อาจารย์ในสาขา',
'Other_academic_works_outsider' => 'ผู้จัดทำร่วม (นอกสาขา)',

'Other_academic_works_Patent' => 'สิทธิบัตร',
'Other_academic_works_Patent_Invention' => 'สิทธิบัตร (การประดิษฐ์)',
'Other_academic_works_Patent_ProductDesign' => 'สิทธิบัตร (การออกแบบผลิตภัณฑ์)',
'Other_academic_works_UtilityModel' => 'อนุสิทธิบัตร',
'Other_academic_works_Copyright' => 'ลิขสิทธิ์',
'Other_academic_works_Copyright_LiteraryWork' => 'ลิขสิทธิ์ (วรรณกรรม)',
'Other_academic_works_Copyright_Music' => 'ลิขสิทธิ์ (ดนตรีกรรม)',
'Other_academic_works_Copyright_Film' => 'ลิขสิทธิ์ (ภาพยนตร์)',
'Other_academic_works_Copyright_Art' => 'ลิขสิทธิ์ (ศิลปกรรม)',
'Other_academic_works_Copyright_Broadcast' => 'ลิขสิทธิ์ (งานแพร่เสี่ยงแพร่ภาพ)',
'Other_academic_works_Copyright_AudioVisualWork' => 'ลิขสิทธิ์ (โสตทัศนวัสดุ)',
'Other_academic_works_Copyright_Other_Works' => 'ลิขสิทธิ์ (งานอื่นใดในแผนกวรรณคดี/วิทยาศาสตร์/ศิลปะ)',
'Other_academic_works_Copyright_SoundRecording' => 'ลิขสิทธิ์ (สิ่งบันทึกเสียง)',
'Other_academic_works_other' => 'อื่น ๆ',
'Other_academic_works_Trade_Secret' => 'ความลับทางการค้า',
'Other_academic_works_Trade_Mark' => 'เครื่องหมายการค้า',


// ส่วนของ ผลงานวิชาการอื่นๆ (edit.blade.php)
'Edit_other_academic_works' => 'แก้ไขผลงานวิชาการอื่นๆ',
'Edit_other_academic_works_detail' => 'แก้ไขรายละเอียดผลงานวิชาการอื่นๆ',

// ========================================== ส่วนของ Admin (Administrator) ==========================================
// Export navbar
'Export_navbar_title' => 'ส่งออก',

// Admin navbar
'Admin_navbar_title' => 'ผู้ดูแลระบบ',

// ส่วนของ User
'User_navbar_title' => 'ผู้ใช้',

// ส่วนของ User (index.blade.php)
'Add_user' => 'เพิ่มผู้ใช้',
'Import_new_user' => 'นำเข้าผู้ใช้ใหม่',
'User_name' => 'ชื่อผู้ใช้',
'User_department' => 'สาขาวิชา',
'User_email' => 'อีเมล',
'User_role' => 'บทบาท',
'User_action' => 'ดำเนินการ',

// ส่วนของ User (import.blade.php)
'Import_Excel_CSV_File' => 'นำเข้าไฟล์ Excel หรือ CSV',
'Choose_file' => 'เลือกไฟล์',

// ส่วนของ User (create.blade.php)
'Create_user' => 'สร้างผู้ใช้',
'Input_user_detail' => 'กรอกรายละเอียดผู้ใช้',
'User_fname_th' => 'ชื่อ (ไทย)',
'User_lname_th' => 'นามสกุล (ไทย)',
'User_fname_en' => 'ชื่อ (อังกฤษ)',
'User_lname_en' => 'นามสกุล (อังกฤษ)',
'User_fname_cn' => 'ชื่อ (จีน)',
'User_lname_cn' => 'นามสกุล (จีน)',
'User_email' => 'อีเมล',
'User_passwrord' => 'รหัสผ่าน',
'User_confirm_password' => 'ยืนยันรหัสผ่าน',
'User_role' => 'บทบาท',
'User_department' => 'สาขาวิชา',
'User_program' => 'หลักสูตร',
'User_select_category' => 'เลือกสาขาวิชา',
'User_select_sub_category' => 'เลือกหลักสูตร',

// ส่วนของ User (show.blade.php)
'User_detail' => 'รายละเอียดผู้ใช้',
'User_description' => 'รายละเอียดของผู้ใช้',
'User_academic_rank' => 'ตำแหน่งทางวิชาการ',
'User_edu_background' => 'วุฒิการศึกษา',
'User_full_name_th' => 'ชื่อเต็ม (ไทย)',
'User_full_name_en' => 'ชื่อเต็ม (อังกฤษ)',
'User_full_name_cn' => 'ชื่อเต็ม (จีน)',

// ส่วนของ User (edit.blade.php)
'Edit_user' => 'แก้ไขผู้ใช้',
'Edit_user_detail' => 'แก้ไขรายละเอียดผู้ใช้',
'User_status' => 'สถานะ',
'User_status_studying' => 'กำลังศึกษา',
'User_status_graduated' => 'จบการศึกษา',

// ส่วนของ Roles
'Role_navbar_title' => 'บทบาท',

// ส่วนของ Role (index.blade.php)
'Add_role' => 'เพิ่ม',
'Role_name' => 'ชื่อบทบาท',
'Role_action' => 'ดำเนินการ',

// ส่วนของ Role (show.blade.php)
'Role_detail' => 'รายละเอียดบทบาท',
'Role_permission' => 'สิทธิ์การใช้งาน',

// ส่วนของ Role (create.blade.php)
'Create_role' => 'สร้างบทบาท',

// ส่วนของ Role (edit.blade.php)
'Edit_role' => 'แก้ไขบทบาท',

// ส่วนของ Permission
'Permission_navbar_title' => 'สิทธิ์การใช้งาน',

// ส่วนของ Permission (index.blade.php)
'Add_permission' => 'เพิ่ม',
'Permission_name' => 'ชื่อสิทธิ์การใช้งาน',
'Permission_action' => 'ดำเนินการ',

// ส่วนของ Permission (create.blade.php)
'Create_permission' => 'สร้างสิทธิ์การใช้งาน',

// ส่วนของ Permission (edit.blade.php)
'Edit_permission' => 'แก้ไขสิทธิ์การใช้งาน',

// ส่วนของ Department
'Department_navbar_title' => 'สาขาวิชา',

// ส่วนของ Department (index.blade.php)
'Add_department' => 'เพิ่มสาขาวิชา',
'Department_name' => 'ชื่อสาขาวิชา',
'Department_action' => 'ดำเนินการ',

// ส่วนของ Department (show.blade.php)
'Deapartment_Name_TH' => 'ชื่อสาขาวิชา (ไทย)',
'Deapartment_Name_EN' => 'ชื่อสาขาวิชา (อังกฤษ)',
'Deapartment_Name_CN' => 'ชื่อสาขาวิชา (จีน)',

// ส่วนของ Department (create.blade.php)
'Create_department' => 'สร้างสาขาวิชา',

// ส่วนของ Department (edit.blade.php)
'Edit_department' => 'แก้ไขสาขาวิชา',

// ส่วนของ Manage Program
'Manage_Program_navbar_title' => 'จัดการหลักสูตร',

// ส่วนของ Manage Program (index.blade.php)
'Add_program' => 'เพิ่ม',
'Program_no' => 'ลำดับที่',
'Program_name' => 'ชื่อหลักสูตร',
'Program_degree' => 'ระดับการศึกษา',
'Program_action' => 'ดำเนินการ',
'Program_name_th' => 'ชื่อหลักสูตร (ไทย)',
'Program_name_en' => 'ชื่อหลักสูตร (อังกฤษ)',
'Program_name_cn' => 'ชื่อหลักสูตร (จีน)',
'Add_new_program' => 'เพิ่มหลักสูตรใหม่',
'Edit_program' => 'แก้ไขหลักสูตร',
'Program_entry_deleted' => 'ลบข้อมูลเรียบร้อยแล้ว',

// ส่วนของ Manage Expertise
'Manage_Expertise_navbar_title' => 'จัดการความเชี่ยวชาญ',

// ส่วนของ Manage Expertise (index.blade.php)
'Expertise_no' => 'ลำดับที่',
'Expertise_teacher_name' => 'ชื่ออาจารย์',
'Expertise_name' => 'ความเชี่ยวชาญ',
'Expertise_action' => 'ดำเนินการ',
'Add_new_expertise' => 'เพิ่มความเชี่ยวชาญใหม่',
'Edit_expertise' => 'แก้ไขความเชี่ยวชาญ',
//researchprofiles page
'no' => 'ลำดับที่',
'year' => 'ปี',
'papername' => 'ชื่อเอกสาร',
'author' => 'ผู้เขียน',
'documenttype' => 'ประเภทเอกสาร',
'page' => 'หน้า',
'journals' => 'วารสาร/ธุรกรรม',
'citations' => 'การอ้างอิง',
'doi' => 'doi',
'placeofpublication' => 'สถานที่ตีพิมพ์',
'book' => 'หนังสือ',
'bookseries' => 'ชุดหนังสือ',
'conferenceproceeding' => 'เอกสารประชุมวิชาการ',
'journal' => 'วารสาร',

//work type
'patent' => 'สิทธิบัตร',
'patentinvention' => 'สิทธิบัตร (การประดิษฐ์)',
'patentdesign' => 'สิทธิบัตร (การออกแบบผลิตภัณฑ์)',
'pettypatent' => 'อนุสิทธิบัตร',
'copyright' => 'ลิขสิทธิ์',
'copyrightliterature' => 'ลิขสิทธิ์ (วรรณกรรม)',
'copyrightmusic' => 'ลิขสิทธิ์ (ตนตรีกรรม)',
'copyrightfilm' => 'ลิขสิทธิ์ (ภาพยนตร์)',
'copyrightart' => 'ลิขสิทธิ์ (ศิลปกรรม)',
'copyrightbroadcast' => 'ลิขสิทธิ์ (งานแพร่เสี่ยงแพร่ภาพ)',
'copyrightaudiovisual' => 'ลิขสิทธิ์ (โสตทัศนวัสดุ)',
'copyrightother' => 'ลิขสิทธิ์ (งานอื่นใดในแผนกวรรณคดี/วิทยาศาสตร์/ศิลปะ)',
'copyrightsoundrecording' => 'ลิขสิทธิ์ (สิ่งบันทึกเสียง)',
'tradesecret' => 'ความลับทางการค้า',
'trademark' => 'เครื่องหมายการค้า',
// ส่วนของ Login
'Account_Login' => 'เข้าสู่ระบบ',
'Username' => 'ชื่อผู้ใช้',
'Password' => 'รหัสผ่าน',
'Remember_me' => 'จดจำบัญชีผู้ใช้',
'Login_button' => 'เข้าสู่ระบบ',
'Forgot_password' => '*** หากลืมรหัสผ่าน ให้ติดต่อผู้ดูแลระบบ',
'Username_kkumail' => 'สำหรับชื่อผู้ใช้ ใช้ KKU-Mail ในการเข้าสู่ระบบ',
'Student_first_time_login' => 'สำหรับนักศึกษาที่เข้าระบบเป็นครั้งแรกให้เข้าสู่ระบบด้วยรหัสนักศึกษา',
'Login_failed' => ' เข้าสู่ระบบล้มเหลว: ชื่อผู้ใช้หรือรหัสผ่านของคุณไม่ถูกต้อง',
'required' => [
        'username' => 'ต้องระบุชื่อผู้ใช้',
        'password' => 'ต้องระบุรหัสผ่าน'
],
'admin' => 'ผู้ดูแลระบบ',
    'teacher' => 'อาจารย์',
    'student' => 'นักศึกษา',
    'staff' => 'เจ้าหน้าที่',
    'headproject' => 'หัวหน้ากลุ่ม',
];



